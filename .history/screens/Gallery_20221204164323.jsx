...
const Gallery = ({ navigation }) => {
    ...
    const changeDisplay = () => setColumns(columns === 5 ? 1 : 5)

    return (
        <View style={styles.container}>
            <View style={styles.headlineBtns}>
                <CustomButtonEmp title={'layout'} onPress={changeDisplay} />
                <CustomButtonEmp title={'camera'} onPress={() => navigation} />
                <CustomButtonEmp title={'delete'} onPress={() => console.log('delete')} />
            </View>
            <View style={styles.gallery}>
                {/* <Text style={styles.text}>Display</Text> */}
                <FlatList numColumns={columns} key={5} data={imageGallery} renderItem={({ item }) => (
